...
import AuthentikIcon from "../components/authentik-icon"

const AUTHENTIK_AUTH_PROVIDER = "authentik"
...
const LoginWithAuthentik = () => {
  ...
  const result = await sdk.auth.login("user", AUTHENTIK_AUTH_PROVIDER, {})
  ...
  const validateCallback = async () => {
    ...
    if (!userExists) {
        // Create user
        await sdk.client.fetch("/authentik/users", {
        method: "POST",
        body: {
    ...
    <>
        ...
        >
        <AuthentikIcon className="h-5 w-5 max-h-5 max-w-5"/>
...
export default LoginWithAuthentik